...
void Server::checker() 
{
	...
	for (std::vector<User>::iterator itb = this->beginVectorUser(); itb != this->endVectorUser(); itb++)
	{
		...
		for (std::vector<std::string>::iterator itv = msg.begin(); itv != msg.end(); itv++)
		{
			...
			switch (itb->getFirstConnexion())
			{
				case true :
				{

					this->firstConnexion(*itb, *itv);
					break ;
				}
				case false :
				{
...
void Server::firstConnexion(User &user, std::string message) 
{
	static int allow = 0; 
	std::vector<std::string> data = this->split(message, ' ');
	...
	if (allow == 1)
	{
		if (data[0].compare("PASS") == 0)
			...
			this->nickCmd(user, message, 1);
		else if (data[0].compare("USER") == 0)
			this->userCmd(user, message, 1);
