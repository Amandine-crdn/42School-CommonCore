...
void Server::intercept()
{
	...
	for (std::vector<pollfd>::iterator itb = ++poll_fds.begin(); itb != poll_fds.end(); itb++)
	{ // skipping the SocketFd
		if (itb->revents == POLLIN)
		{
			...
			else
			{
				...
				this->getUserByFd(itb->fd).setFd(itb->fd);
				checker(this->getUserByFd(itb->fd), this->getUserByFd(itb->fd).getMsg());
				this->getUserByFd(itb->fd).eraseMsg();
			}
		}
	}
}   
