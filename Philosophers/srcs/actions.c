...
/*   Updated: 2022/04/12 12:03:17 by acerdan          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_think(t_philo *philo)
{
	...
	printf("%ld		%d is thinking\n",
		ft_get_time() -  philo->p->start_time, philo->index); // difference entre le moment pù le repas a commencer et l'heure de maintenant ??
	pthread_mutex_unlock(&philo->p->msg_m);
	//si le philo a pas encore manger et que sa place est paire... il dort ??
	//on commence par les impairs
	if (philo->nb_meal == 0 && philo->index % 2 == 0)
		usleep(10000); //prk usleep 1 sec et pas autre?
